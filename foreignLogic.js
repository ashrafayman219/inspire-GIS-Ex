...
function initializeBasemapPanel() {
  const basemapItems = document.querySelectorAll('#sidePanelContent .basemap-item');
...
async function handleFiles(files) {
  for (let file of files) {
    ...
    if (!['csv', 'geojson', 'json'].includes(extension)) {
      ...
      showNotification('Error: Only CSV and GeoJSON files are supported', 'error');
      continue;
    }
...
function updateLayerList() {
  ...
  if (uploadedLayers.length === 0) {
    ...
    targetList.innerHTML = uploadedLayers.map((layer, index) => `
...
function initializeDrawingPanel() {
  ...
  const drawToolBtns = document.querySelectorAll('#sidePanelContent .draw-tool-btn');
  ...
  const opacityValue = document.querySelector('#sidePanelContent .slider-value');
...
function initializeDrawingToolButtons() {
  ...
  document.getElementById('drawColor').addEventListener('change', updateActiveGraphicsSymbology);
  document.getElementById('drawOpacity').addEventListener('input', updateActiveGraphicsSymbology);
}
...
function startDrawingWithTool(tool) {
  ...
  const color = hexToRgb(document.getElementById('drawColor')?.value || '#2196F3');
...
function initializeCoordinateDisplay() {
  ...
  view.on('pointer-move', (event) => {
    ...
    if (point) {
      ...
      document.getElementById('coordDisplay').textContent = `Lat: ${currentCoords.lat}, Lon: ${currentCoords.lon}`;
    }
  });
    ...
    if (results.length > 0) {
      ...
      if (graphic.layer && graphic.layer.queryFeatures) {
        try {
          ...
          console.warn('Could not query feature, using hitTest result:', queryError);
          showCustomPopup(graphic, event.mapPoint);
        }
      } else {
        showCustomPopup(graphic, event.mapPoint);
...
async function handleCountryClick(event) {
  ...
  try {
    const response = await view.hitTest(event);
    const uploadedFeatureHit = response.results.find(result => 
      result.graphic && 
      result.graphic.layer && 
...
async function getSuggestions(searchTerm) {
  ...
  async function selectSuggestion(suggestion) {
    ...
    try {
      if (suggestion.feature || suggestion.extent) {
        ...
        const response = await searchWidget.viewModel.search(suggestion.suggestResult);
  ...
  async function addSearchMarker(feature) {
    ...
    const graphic = new Graphic({
      ...
      popupTemplate: {
        title: feature.attributes.PlaceName || feature.attributes.name || "Search Result",
        content: feature.attributes.Place_addr || feature.attributes.address || "Location"
      }
    });
  ...
  function getIconForSuggestion(suggestion) {
    ...
    if (text.includes('restaurant') || text.includes('food')) return 'fas fa-utensils';
    if (text.includes('hotel') || text.includes('lodging')) return 'fas fa-bed';
    if (text.includes('park')) return 'fas fa-tree';
    if (text.includes('school') || text.includes('university')) return 'fas fa-graduation-cap';
    if (text.includes('hospital') || text.includes('medical')) return 'fas fa-hospital';
    if (text.includes('shopping') || text.includes('store')) return 'fas fa-shopping-cart';
  ...
  function showSuggestionsLoading() {
    suggestionsDiv.innerHTML = '<div class="search-loading"><i class="fas fa-spinner fa-spin"></i> Searching...</div>';
    suggestionsDiv.classList.add('active');
  }
  ...
  searchInput.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown' && suggestionItems.length > 0) {
      ...
      updateSelectedSuggestion(suggestionItems);
    } else if (e.key === 'Enter' && selectedSuggestionIndex >= 0 && currentSuggestions.length > 0) {
      e.preventDefault();
...
function showCustomPopup(graphic, mapPoint) {
  ...
  if (attributes && Object.keys(attributes).length > 0) {
    ...
    const sortedKeys = Object.keys(attributes).sort((a, b) => {
      ...
      const endFields = ['shape_area', 'shape__area', 'shape_length', 'shape__length', 'shape_leng', 'objectid', 'fid'];
      ...
      case 'point':
        html = `
          ...
          <div class="attribute-row">
            ...
            <span class="attribute-value">${geometry.longitude.toFixed(6)}</span>
          </div>
          <div class="attribute-row">
...
// Continue the polygon case and complete the updateGeometryDetails function
    ...
    detailsDiv.innerHTML = '<div class="error-message">Unable to calculate geometry details</div>';
  }
}
...
window.addEventListener('resize', () => {
  ...
  resizeTimeout = setTimeout(() => {
    ...
    // Adjust mobile UI
    if (window.innerWidth < 768 && controlPanel && !controlPanel.classList.contains('collapsed')) {
      togglePanel(true);
    }
  }, 250);
});
...
async function toggleCompass() {
  if (!activeWidgets.has('compass')) {
    const [Compass] = await Promise.all([
  ...
  if (!measurementWidget) {
    ...
    measurementWidget.watch('activeTool', (tool) => {
      if (!tool) {
        ...
        if (content) {
          content.innerHTML = '<div style="color: var(--text-secondary);">Select a measurement tool</div>';
        }
      }
    });
  }
  ...
  if (measurement.distance !== undefined) {
    ...
    html += `
      <div class="measurement-item">
        ...
        <span class="measurement-value">${measurement.distance.toFixed(2)} ${measurement.distanceUnit}</span>
      </div>
    `;
    ...
    html += `
      <div class="measurement-item">
        ...
        <span class="measurement-value">${measurement.area.toFixed(2)} ${measurement.areaUnit}</span>
      </div>
      <div class="measurement-item">
        <i class="fas fa-draw-polygon"></i>
        <span class="measurement-label">Perimeter:</span>
        <span class="measurement-value">${measurement.perimeter.toFixed(2)} ${measurement.perimeterUnit}</span>
      </div>
    `;
  }
...
function renderTable() {
  ...
  const columns = Object.keys(filteredData[0])
    .filter(key => !key.startsWith('_') && key !== 'ObjectID' && key !== 'FID');
  ...
  columns.forEach(col => {
    ...
    html += `<th class="${sortClass}" onclick="sortTable('${col}')">${formatFieldName(col)}</th>`;
  });
  html += '</tr></thead>';
...
function updateHeatmapLayerSelect() {
  ...
  uploadedLayers.forEach((layer, index) => {
    ...
    if (layer.geometryType === 'point' || 
        (layer.source && layer.source.length > 0 && layer.source.items[0]?.geometry?.type === 'point')) {
      const option = document.createElement('option');
      option.value = index;
      option.textContent = layer.title;
      select.appendChild(option);
  ...
  if (layer.fields) {
    layer.fields.forEach(field => {
      if (field.type === 'double' || field.type === 'integer' || 
          field.type === 'single' || field.type === 'small-integer') {
        const option = document.createElement('option');
...
function initializeClassificationPanel() {
  ...
  layerSelect.addEventListener('change', async (e) => {
    ...
    if (layer.fields) {
      layer.fields.forEach(field => {
        if (field.type === 'double' || field.type === 'integer' || 
            field.type === 'single' || field.type === 'small-integer' || 
            field.type === 'string' || field.type === 'oid') {
          const option = document.createElement('option');
          option.value = field.name;
          option.textContent = formatFieldName(field.name);
  ...
  fieldSelect.addEventListener('change', async (e) => {
    ...
    if (fieldName && currentClassificationLayer) {
      const stats = await analyzeFieldForClassification(currentClassificationLayer, fieldName);
...
function generateClassificationColors(count) {
  const colorSchemes = [
    // Blue to Red
    [[56, 149, 211], [65, 171, 93], [255, 204, 0], [255, 127, 0], [215, 25, 28]],
    // Purple to Orange  
    [[94, 79, 162], [158, 154, 200], [247, 247, 247], [253, 174, 97], [244, 109, 67]],
    // Green gradient
    [[247, 252, 245], [199, 233, 192], [120, 198, 121], [49, 163, 84], [0, 109, 44]],
    // Spectral
    [[215, 48, 39], [252, 141, 89], [254, 224, 139], [145, 191, 219], [69, 117, 180]]
  ];
...
async function applyClassification() {
  ...
  try {
    ...
    if (!originalRenderers.has(currentClassificationLayer)) {
      originalRenderers.set(currentClassificationLayer, currentClassificationLayer.renderer);
    }
...
async function executeBuffer() {
  ...
  try {
    ...
    const isUsingLayer = activeButton && activeButton.textContent.includes('From Layer');
    
    console.log('Active button:', activeButton?.textContent, 'Using layer:', isUsingLayer);
    ...
    closeBufferModal();
    showNotification(`Created ${analysisLayer.graphics.length} buffer(s)`, 'success');
...
window.debugModals = function() {
  ...
  console.log('Buffer Modal:', {
    ...
    visibility: bufferModal ? window.getComputedStyle(bufferModal).visibility : 'N/A',
  ...
  console.log('Intersect Modal:', {
    ...
    display: intersectModal ? window.getComputedStyle(intersectModal).display : 'N/A',
    visibility: intersectModal ? window.getComputedStyle(intersectModal).visibility : 'N/A',
    zIndex: intersectModal ? window.getComputedStyle(intersectModal).zIndex : 'N/A'
  });
};
...
function setIntersectSource(featureNum, source) {
  ...
  const settingGroups = document.querySelectorAll('#intersectModal .setting-group');
  const targetGroup = featureNum === 1 ? settingGroups[1] : settingGroups[2];
  const buttons = targetGroup.querySelectorAll('.source-btn');
  
  buttons.forEach(btn => {
    btn.classList.remove('active');
    if ((source === 'layer' && btn.textContent.includes('Layer')) ||
        (source === 'draw' && btn.textContent.includes('Draw'))) {
      btn.classList.add('active');
    }
  });
  
  const layerSection = document.getElementById(`intersectLayer${featureNum}Section`);
  const drawSection = document.getElementById(`intersectDraw${featureNum}Section`);
...
async function executeIntersection() {
  try {
    ...
    const firstSectionButtons = featureSections[0].querySelectorAll('.button-group-item');
    const source1ActiveBtn = Array.from(firstSectionButtons).find(btn => btn.classList.contains('active'));
    const source1IsLayer = source1ActiveBtn && source1ActiveBtn.textContent.includes('From Layer');
    
    console.log('Source 1 - Active button:', source1ActiveBtn?.textContent, 'Is layer:', source1IsLayer);
    ...
    const secondSectionButtons = featureSections[1].querySelectorAll('.button-group-item');
    const source2ActiveBtn = Array.from(secondSectionButtons).find(btn => btn.classList.contains('active'));
    const source2IsLayer = source2ActiveBtn && source2ActiveBtn.textContent.includes('From Layer');
    
    console.log('Source 2 - Active button:', source2ActiveBtn?.textContent, 'Is layer:', source2IsLayer);
    ...
    const keepNonIntersecting = document.getElementById('keepNonIntersecting').checked;
    ...
    if (keepNonIntersecting && allIntersections.length > 0) {
      ...
      let unionedIntersections = allIntersections.length > 1 ? 
      ...
      features1.forEach((feature1, index) => {
        try {
          const difference = geometryEngine.difference(feature1.geometry, unionedIntersections);
          if (difference) {
            const graphic = new Graphic({
              geometry: difference,
              symbol: {
      ...
      features2.forEach((feature2, index) => {
        try {
          const difference = geometryEngine.difference(feature2.geometry, unionedIntersections);
          if (difference) {
            const graphic = new Graphic({
              geometry: difference,
              symbol: {
...
window.setDistanceSource = async function(source) {
  const buttons = document.querySelectorAll('#distancePanel .button-group-item');
  ...
  const distanceGraphics = analysisLayer.graphics.filter(g => 
    g.attributes && g.attributes.type === "Distance Measurement"
  );
  analysisLayer.removeMany(distanceGraphics.toArray());
  
  if (source === 'draw') {
    helpText.textContent = 'Click on map to place two points for distance measurement';
...
window.setIntersectSource = function(featureNum, source) {
  ...
  const activeButton = Array.from(buttons).find(btn => {
    ...
    return (source === 'layer' && buttonText.includes('layer')) ||
...
async function handleDistanceClick(event) {
  ...
  const response = await view.hitTest(event);
  const results = response.results.filter(r => r.graphic && r.graphic.layer && r.graphic.geometry);
  ...
  if (results.length > 0) {
    ...
    const selectedGraphic = new Graphic({
      ...
      symbol: {
        type: results[0].graphic.geometry.type === 'point' ? 'simple-marker' : 'simple-fill',
        color: [255, 0, 255, 0.3],
        outline: {
          color: [255, 0, 255, 1],
          width: 2
    ...
    if (distanceFeatures.length === 1) {
      showNotification('First feature selected. Click another feature.', 'info');
    } else if (distanceFeatures.length === 2) {
      // Clear selection graphics
      const selectionGraphics = view.graphics.items.filter(g => 
        g.attributes && g.attributes.type === "Distance Selection"
      );
...
async function calculateDistance() {
  try {
    ...
    const point1 = geom1.type === 'point' ? geom1 : geom1.centroid || geom1.extent.center;
    const point2 = geom2.type === 'point' ? geom2 : geom2.centroid || geom2.extent.center;
    ...
    const previousLines = analysisLayer.graphics.filter(g => 
      g.attributes && g.attributes.type === "Distance Measurement"
    );
    ...
    const helpText = document.getElementById('distanceHelp');
    if (helpText) {
      helpText.textContent = 'Distance calculated. Click two more features to measure again.';
    }
...
async function startAreaAnalysis() {
  ...
  const polygonLayers = uploadedLayers.filter(layer => 
    layer.geometryType === 'polygon' || 
    (layer.graphics && layer.graphics.length > 0 && 
  ...
  try {
    ...
    showNotification(`Total area of ${featureCount} polygons: ${areaText}`, 'success');
...
async function setupTimeFields(layerIndex) {
  ...
  // Get date fields
  if (layer.fields) {
    const dateFields = layer.fields.filter(field => 
      field.type === 'date' || 
...
function playTimeAnimation() {
  ...
  if (timeAnimation) {
    ...
    timeAnimation = setInterval(() => {
      ...
      const nextEnd = new Date(currentEnd.getTime() + interval.value * 86400000); // Convert days to ms
      ...
      if (nextEnd <= fullExtent.end) {
        ...
        timeSlider.timeExtent = {
          ...
          end: new Date(fullExtent.start.getTime() + (currentEnd - timeSlider.timeExtent.start))
        };
      }
    }, animationDelay);
  }
...
window.toggleModalMinimize = function(modalId) {
  ...
  if (modal.classList.contains('minimized')) {
    ...
    indicator.innerHTML = `
      <span>${modalId === 'bufferModal' ? 'Buffer' : 'Intersection'} Analysis</span>
...
async function startBufferDrawing(type) {
  ...
  if (!document.getElementById('drawingIndicator')) {
    ...
    indicator.innerHTML = `
      ...
      <span>Drawing ${type}. ${type === 'point' ? 'Click to place point' : 'Click to add vertices, double-click to complete'}.</span>
...
function stopAnalysisDrawing() {
  ...
  document.querySelectorAll('.draw-mini-btn').forEach(btn => btn.classList.remove('active'));
}
...
function startAppTour() {
  ...
  if (typeof Shepherd === 'undefined') {
    showNotification('Tour library not loaded. Please refresh the page.', 'error');
    return;
  }
...
tour.addStep({
  title: '<i class="fas fa-globe-americas"></i> Welcome to GIS Studio!',
